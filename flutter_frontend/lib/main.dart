...
import 'dashboard_page.dart'; // New: Import the http package
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        useMaterial3: true,
        fontFamily:
            'Archivo', // Assuming you add the font to pubspec, otherwise falls back to default
...
class _LoginPageState extends State<LoginPage> with TickerProviderStateMixin {
  ...
  // New: Define your API endpoint here
  // Use the local backend when running on the emulator. For Android, 10.0.2.2 loops back to host.
  static const String _loginApiUrl = 'http://10.0.2.2:3000/api/user/login';
  static const String _signupApiUrl = 'http://10.0.2.2:3000/api/user/signup';
  static const String _forgotApiUrl = 'http://10.0.2.2:3000/api/user/forgotPassword';
  ...
  // UPDATED: Handle login with API call
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        final response = await http.post(
          ...
          // Backend expects username/password (not email) for login
        ...
        if (response.statusCode == 200) {
          ...
          // Decode the response body (e.g., to get a user token or profile data)
          final Map<String, dynamic> responseData = jsonDecode(response.body);
          final String token =
              responseData['token'] ?? 'No Token'; // Example of extracting data

          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text("Welcome, Stargazer! Token: $token"),
                backgroundColor: const Color(0xFF4158d0),
              ),
            );
            // TODO: Navigate to main application screen (e.g., Navigator.push...)
          ...
          // Failure: Unauthorized (e.g., bad credentials)
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
          ...
          // Failure: Server error or other issue
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  "Error ${response.statusCode}: Could not connect to the cosmos.",
                ),
        ...
        // 4. Handle connection errors (network down, bad URL, etc.)
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                "Network Error: Could not reach the server. Details: $e",
              ),
              ...
              onPressed: () async {
                ...
                try {
                  ...
                  if (res.statusCode == 200) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Reset failed: ${res.body}'),
                        backgroundColor: Colors.red,
                      ),
                    );
                  }
              ...
              onPressed: () async {
                ...
                try {
                  ...
                  if (res.statusCode == 201 || res.statusCode == 200) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Sign up failed: ${res.body}'),
                        backgroundColor: Colors.red,
                      ),
                    );
                  }
                          ...
                          child: const Text(
                            ...
                            style: TextStyle(
                              ...
                              decoration: TextDecoration
                                  .none, // Removed underline by default
                            ),
                          ),
                        ),
                      ),
